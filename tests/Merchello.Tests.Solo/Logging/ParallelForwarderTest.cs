namespace Merchello.Tests.Solo.Logging
{
    using System;
    ...
    public class ParallelForwarderTest : IDisposable
    {
        ...
        private Logging.DebugAppender debugAppender;
        ...
        {
            this.debugAppender = new Logging.DebugAppender();
            this.debugAppender.ActivateOptions();

            this.asyncForwardingAppender = new ParallelForwardingAppender();
            this.asyncForwardingAppender.AddAppender(this.debugAppender);
            this.asyncForwardingAppender.ActivateOptions();

            this.repository = LogManager.CreateRepository(Guid.NewGuid().ToString());
            BasicConfigurator.Configure(this.repository, this.asyncForwardingAppender);

            this.log = LogManager.GetLogger(this.repository.Name, "TestLogger");
        }
        ...
        public void CanHandleNullLoggingEvent()
        {
            ...
            // Act
            this.asyncForwardingAppender.DoAppend((LoggingEvent)null);
            this.log.Info("SusequentMessage");
            this.asyncForwardingAppender.Close();

            // Assert - should not have had an exception from previous call
            Assert.That(this.debugAppender.LoggedEventCount, Is.EqualTo(1), "Expected subsequent message only");
            Assert.That(this.debugAppender.GetEvents()[0].MessageObject, Is.EqualTo("SusequentMessage"));
        }
        ...
        public void CanHandleAppenderThrowing()
        {
            ...
            this.asyncForwardingAppender.AddAppender(badAppender.Object);
            ...
            // Assert
            Assert.That(this.debugAppender.LoggedEventCount, Is.EqualTo(2));
            Assert.That(this.debugAppender.GetEvents()[1].MessageObject, Is.EqualTo("SusequentMessage"));
        ...
        public void WillLogFastWhenThereIsASlowAppender()
        {
            ...
            // Arrange
            this.debugAppender.AppendDelay = TimeSpan.FromSeconds(30);
            ...
            // Assert
            Assert.That(this.debugAppender.LoggedEventCount, Is.EqualTo(0));
        ...
        public void WillNotOverflow()
        {
            ...
            // Arrange
            this.debugAppender.AppendDelay = TimeSpan.FromMilliseconds(1);
            this.asyncForwardingAppender.BufferSize = 100;
            ...
            while (this.asyncForwardingAppender.BufferEntryCount > 0) ;
            this.asyncForwardingAppender.Close();

            // Assert
            Assert.That(this.debugAppender.LoggedEventCount, Is.EqualTo(testSize));
        }
        ...
        public void WillTryToFlushBufferOnShutdown()
        {
            ...
            var numberLoggedBeforeClose = this.debugAppender.LoggedEventCount;
            this.asyncForwardingAppender.Close();
            var numberLoggedAfterClose = this.debugAppender.LoggedEventCount;
        ...
        public void WillShutdownIfBufferCannotBeFlushedFastEnough()
        {
            ...
            // Arrange
            this.debugAppender.AppendDelay = TimeSpan.FromSeconds(1);
            ...
            var events = this.debugAppender.GetEvents();
        ...
        public void ThreadContextPropertiesArePreserved()
        {
            ...
            Assert.That(this.asyncForwardingAppender.Fix & FixFlags.Properties, Is.EqualTo(FixFlags.Properties), "Properties must be fixed if they are to be preserved");
            ...
            // Assert
            var lastLoggedEvent = this.debugAppender.GetEvents()[0];
        ...
        public void MessagesExcludedByFilterShouldNotBeAppended()
        {
            ...
            this.asyncForwardingAppender.AddFilter(levelFilter);
        ...
        protected virtual void Dispose(bool disposing)
        {
            if (!this._disposed)
            {
                if (disposing)
                {
                    if (this.asyncForwardingAppender != null)
                    {
                        this.asyncForwardingAppender.Dispose();
                        this.asyncForwardingAppender = null;
                    }
                }

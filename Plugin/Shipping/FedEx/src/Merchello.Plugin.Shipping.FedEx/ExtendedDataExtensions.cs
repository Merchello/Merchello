...
namespace Merchello.Plugin.Shipping.FedEx
{
    public static class ExtendedDataExtensions
    {
        ...
        public static FedExProcessorSettings GetProcessorSettings(this ExtendedDataCollection extendedData)
        {
            if (!extendedData.ContainsKey(Constants.ExtendedDataKeys.ProcessorSettings)) return new FedExProcessorSettings();
        
            return
                JsonConvert.DeserializeObject<FedExProcessorSettings>(

namespace Merchello.Core.Acquired.Plugins
{
    ...
    internal static class TypeHelper
    {
        ...
        /// </summary>
        /// <param name="assignTypeFrom">The type</param>
        /// <param name="assemblies">The assemblies to check</param>
        /// <returns>The collection of matching assemblies</returns>
        /// <remarks>
        ...
        public static Assembly[] GetReferencedAssemblies(Type assignTypeFrom, IEnumerable<Assembly> assemblies)
        {
            ...
            return
                assemblies.Where(
                    ...
                    || HasReferenceToAssemblyWithName(assembly, assignTypeFrom.Assembly.GetName().Name)).ToArray();
        }
        ...
        /// <returns>A value indicating if the class is not static</returns>
        ...
        {
            return CachedDiscoverableProperties(type, mustRead, mustWrite, includeIndexed).FirstOrDefault(
                x =>
                    {
                        if (caseSensitive) return x.Name == name;
        ...
        /// <returns></returns>
        public static PropertyInfo[] CachedDiscoverableProperties(
            Type type,
            bool mustRead = true,
            bool mustWrite = true,
            bool includeIndexed = false)
            ...
            return
                GetPropertiesCache.GetOrAdd(
                    ...
                    type.GetProperties(BindingFlags.Public | BindingFlags.Instance)
                        .Where(
                            y =>
                            (!mustRead || y.CanRead) && (!mustWrite || y.CanWrite)
        ...
        /// <param name="expectedAssemblyName">The expected name</param>
        /// <returns>A value indicating if the names are the same</returns>
        ...
        private static bool MatchGeneric(Type implementation, Type contract, IDictionary<string, Type> bindings)
        {
            ...
            for (var i = 0; i < implArgs.Length; i++)
            {
                ...
                if (MatchType(implArgs[i], contArgs[i], bindings, variance) == false) return false;
            }
        ...
        internal static bool MatchType(
            Type implementation,
            Type contract,
            IDictionary<string, Type> bindings,
            bool variance = true)
        {

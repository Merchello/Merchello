...
namespace Merchello.Core.Acquired.ObjectResolution
{
        ...
        where TResolver : ResolverBase
    {
        ...
        internal ManyObjectsResolverBase(ILogger logger, IEnumerable<Type> types, ObjectLifetimeScope scope = ObjectLifetimeScope.Application)
        {
            ...
            this.CanResolveBeforeFrozen = false;
            this._instanceTypes = types.ToList();
        ...
        internal ManyObjectsResolverBase(ILogger logger, ObjectLifetimeScope scope = ObjectLifetimeScope.Application)
        {
            ...
            this.Logger = logger;
            this.LifetimeScope = scope;            
            this._instanceTypes = new List<Type>();
        }
        ...
        protected ManyObjectsResolverBase(IServiceProvider serviceProvider, ILogger logger, ObjectLifetimeScope scope = ObjectLifetimeScope.Application)
        {
            ...
            this.LifetimeScope = scope;
            if (scope == ObjectLifetimeScope.HttpRequest)
                this._httpContextKey = this.GetType().FullName;
            this._instanceTypes = new List<Type>();

            this.InitializeAppInstances();
        }
        ...
        protected ManyObjectsResolverBase(IServiceProvider serviceProvider, ILogger logger, HttpContextBase httpContext)
        {
            ...
            this.CanResolveBeforeFrozen = false;
            this.Logger = logger;
            this.LifetimeScope = ObjectLifetimeScope.HttpRequest;
            this._httpContextKey = this.GetType().FullName;
            this.ServiceProvider = serviceProvider;
        ...
        {
            this._instanceTypes = value.ToList();
        }
        ...
        {
            this._applicationInstances = new Lazy<IEnumerable<TResolved>>(() => this.CreateInstances().ToArray());
        }
        ...
        protected IEnumerable<TResolved> GetSortedValues()
        {
            if (this._sortedValues == null)
            {
                ...
                values.Sort((f1, f2) => this.GetObjectWeight(f1).CompareTo(this.GetObjectWeight(f2)));
        ...
        {
            get { return this._defaultPluginWeight; }
            set { this._defaultPluginWeight = value; }
        }
        ...
        protected virtual int GetObjectWeight(object o)
        {
            ...
            return attr == null ? this.DefaultPluginWeight : attr.Weight;
        }
        ...
        {
            get { return this.CreateValues(this.LifetimeScope); }
        }
        ...
        protected virtual IEnumerable<TResolved> CreateValues(ObjectLifetimeScope scope)
        {
            using (Resolution.Reader(this.CanResolveBeforeFrozen))
            {
                ...
                switch (this.LifetimeScope)
                {
                    case ObjectLifetimeScope.HttpRequest:

                        // create new instances per HttpContext
                        if (this.CurrentHttpContext.Items[this._httpContextKey] == null)
                        {
                            var instances = this.CreateInstances().ToArray();
                            ...
                            {
                                this.CurrentHttpContext.DisposeOnPipelineCompleted(disposable);
                            }
                            this.CurrentHttpContext.Items[this._httpContextKey] = instances;
                        }
                        return (TResolved[])this.CurrentHttpContext.Items[this._httpContextKey];

                    case ObjectLifetimeScope.Application:

                        return this._applicationInstances.Value;

                    case ObjectLifetimeScope.Transient:
                    default:
                        // create new instances each time
                        return this.CreateInstances().ToArray();
                }
            }
        }
        ...
        {
            return this.ServiceProvider.CreateInstances<TResolved>(this.InstanceTypes, this.Logger);
        }
        ...
        public virtual void RemoveType(Type value)
        {
            ...
            using (var l = new UpgradeableReadLock(this._lock))
            {
                ...
                this._instanceTypes.Remove(value);
            }
        }
        ...
        protected void AddTypes(IEnumerable<Type> types)
        {
            ...
            using (new WriteLock(this._lock))
            {
                ...
                {
                    this.EnsureCorrectType(t);
                    if (this._instanceTypes.Contains(t))
                    {
                        throw new InvalidOperationException(string.Format(
                            "Type {0} is already in the collection of types.", t.FullName));
                    }
                    this._instanceTypes.Add(t);
                }
            }
        }
        ...
        public virtual void AddType(Type value)
        {
            ...
            {
                this.EnsureCorrectType(value);
                if (this._instanceTypes.Contains(value))
                {
                    throw new InvalidOperationException(string.Format(
        ...
        {
            this.EnsureSupportsClear();

            using (Resolution.Configuration)
            using (new WriteLock(this._lock))
            {
                this._instanceTypes.Clear();
            }
        }
        ...
        internal void ResetCollections()
        {
            using (new WriteLock(this._lock))
            {
                ...
                this._applicationInstances = null;
            }
        }
        ...
        public virtual void InsertType(int index, Type value)
        {
            ...
            using (var l = new UpgradeableReadLock(this._lock))
            {
                ...
                l.UpgradeToWriteLock();
                this._instanceTypes.Insert(index, value);
            }
        }
        ...
        public void InsertType<T>(int index)
            where T : TResolved
        {
            this.InsertType(index, typeof(T));
        }
        ...
        public virtual void InsertTypeBefore(Type existingType, Type value)
        {
            ...
            using (var l = new UpgradeableReadLock(this._lock))
            {
                this.EnsureCorrectType(existingType);
                this.EnsureCorrectType(value);
                if (this._instanceTypes.Contains(existingType) == false)
                {
                    throw new InvalidOperationException(string.Format(
                ...
                int index = this._instanceTypes.IndexOf(existingType);
        ...
        public void InsertTypeBefore<TExisting, T>()
            ...
            where T : TResolved
        {
            this.InsertTypeBefore(typeof(TExisting), typeof(T));
        }
        ...
        public virtual bool ContainsType(Type value)
        {
            ...
            {
                return this._instanceTypes.Contains(value);
            }
        }
        ...
        public virtual IEnumerable<Type> GetTypes()
        {
            ...
            using (new ReadLock(this._lock))
            {
                types = this._instanceTypes.ToArray();
            }
            return types;
        }
        ...
        {
            return new WriteLock(this._lock);
        }

...
namespace Merchello.Core.Acquired.ObjectResolution
{
        ...
        where TResolver : ResolverBase
    {
            ...
            : base(logger, scope)
        {
            ...
            this._typeListProducerList.Add(typeListProducerList);
            this.Initialize();
        }
        ...
        {
            this._resolvedTypes = new Lazy<List<Type>>(() =>
            {
                var resolvedTypes = new List<Type>();

                // get the types by evaluating the lazy & producers
                var types = new List<Type>();
                types.AddRange(this._lazyTypeList.Select(x => x.Value));
                types.AddRange(this._typeListProducerList.SelectMany(x => x()));

                // we need to validate each resolved type now since we could
                // not do it before evaluating the lazy & producers
                foreach (var type in types.Where(x => this._excludedTypesList.Contains(x) == false))
                {
                    this.AddValidAndNoDuplicate(resolvedTypes, type);
                }
        ...
        {
            get { return this._resolvedTypes.IsValueCreated; }
        }
        ...
        {
            get { return this._resolvedTypes.Value; }
        }
        ...
        public override void RemoveType(Type value)
        {
            ...
            this._excludedTypesList.Add(value);
        }
        ...
        public void AddTypeListDelegate(Func<IEnumerable<Type>> typeListProducer)
        {
            ...
            using (this.GetWriteLock())
            {
                this._typeListProducerList.Add(typeListProducer);
            }
        }
        ...
        {
            this.AddType(new Lazy<Type>(() => value));
        }

...
namespace Merchello.Core.Acquired.Persistence.Mappers
{
    ...
    internal abstract class BaseExpressionHelper<T> : BaseExpressionHelper
    {
        ...
        protected virtual string VisitLambda(LambdaExpression lambda)
        {
            if (lambda.Body.NodeType == ExpressionType.MemberAccess)
            {
                ...
                if (m.Expression != null)
                {
                    ...
                    string r = this.VisitMemberAccess(m);
                    this.SqlParameters.Add(true);
                    return string.Format("{0} = @{1}", r, this.SqlParameters.Count - 1);
        ...
        {
            string left, right;
            var operand = this.BindOperant(b.NodeType);
            if (operand == "AND" || operand == "OR")
            {
                ...
                if (m != null && m.Expression != null)
                {
                    ...
                    this.SqlParameters.Add(1);
                    left = string.Format("{0} = @{1}", r, this.SqlParameters.Count - 1);
                ...
                if (m != null && m.Expression != null)
                {
                    ...
                    this.SqlParameters.Add(1);
                    right = string.Format("{0} = @{1}", r, this.SqlParameters.Count - 1);
            ...
            else if (operand == "=")
            {
                ...
                if (constRight != null && constRight.Type == typeof (bool))
                    return ((bool) constRight.Value) ? this.VisitNotNot(b.Left) : this.VisitNot(b.Left);
                right = this.Visit(b.Right);

                // deal with (true|false == x) - why not
                var constLeft = b.Left as ConstantExpression;
                if (constLeft != null && constLeft.Type == typeof (bool))
                    return ((bool) constLeft.Value) ? this.VisitNotNot(b.Right) : this.VisitNot(b.Right);
                left = this.Visit(b.Left);
            }
            else if (operand == "<>")
            {
                ...
                var constRight = b.Right as ConstantExpression;
                if (constRight != null && constRight.Type == typeof (bool))
                    return ((bool) constRight.Value) ? this.VisitNot(b.Left) : this.VisitNotNot(b.Left);
                right = this.Visit(b.Right);

                // deal with (true|false != x) - why not
                var constLeft = b.Left as ConstantExpression;
                if (constLeft != null && constLeft.Type == typeof (bool))
                    return ((bool) constLeft.Value) ? this.VisitNot(b.Right) : this.VisitNotNot(b.Right);
                left = this.Visit(b.Left);
            }
            else
            {
        ...
        protected virtual string VisitNew(NewExpression nex)
        {
            ...
            {
                // FieldName ?
                List<Object> exprs = this.VisitExpressionList(nex.Arguments);
        ...
        protected virtual string VisitConstant(ConstantExpression c)
        {
            ...
            this.SqlParameters.Add(c.Value);
            return string.Format("@{0}", this.SqlParameters.Count - 1);
        ...
        protected virtual string VisitUnary(UnaryExpression u)
        {
            ...
            {
                case ExpressionType.Not:
                    return this.VisitNot(u.Operand);
                default:
        ...
        private string VisitNot(Expression exp)
        {
            ...
            switch (exp.NodeType)
            {
                case ExpressionType.MemberAccess:
                    ...
                    this.SqlParameters.Add(true);
                    return string.Format("NOT ({0} = @{1})", o, this.SqlParameters.Count - 1);
                default:
        ...
        private string VisitNotNot(Expression exp)
        {
            ...
            switch (exp.NodeType)
            {
                case ExpressionType.MemberAccess:
                    ...
                    this.SqlParameters.Add(true);
                    return string.Format("({0} = @{1})", o, this.SqlParameters.Count - 1);
                default:
                    // could be anything else, such as: x => x.Path.StartsWith("-20")
                    return o;
            }
        ...
        protected virtual string VisitMethodCall(MethodCallExpression m)
        {
            ...
            var visitedObjectForMethod = this.Visit(objectForMethod);
            ...
            {
                case "ToString":
                    this.SqlParameters.Add(objectForMethod.ToString());
                    return string.Format("@{0}", this.SqlParameters.Count - 1);
                case "ToUpper":
                ...
                case "InvariantEquals":
                    ...
                    return this.HandleStringComparison(visitedObjectForMethod, compareValue, m.Method.Name, colType);
                //case "Substring":
    ...
    internal abstract class BaseExpressionHelper
    {
        ...
        protected string HandleStringComparison(string col, string val, string verb, TextColumnType columnType)
        {
            ...
            {
                case "SqlWildcard":
                    this.SqlParameters.Add(this.RemoveQuote(val));
                    return this.SqlSyntax.GetStringColumnWildcardComparison(col, this.SqlParameters.Count - 1, columnType);
                case "Equals":
                    this.SqlParameters.Add(this.RemoveQuote(val));
                    return this.SqlSyntax.GetStringColumnEqualComparison(col, this.SqlParameters.Count - 1, columnType);
                case "StartsWith":
                    this.SqlParameters.Add(string.Format("{0}{1}",
                        this.RemoveQuote(val),
                        this.SqlSyntax.GetWildcardPlaceholder()));
                    return this.SqlSyntax.GetStringColumnWildcardComparison(col, this.SqlParameters.Count - 1, columnType);
                case "EndsWith":
                    this.SqlParameters.Add(string.Format("{0}{1}",
                        this.SqlSyntax.GetWildcardPlaceholder(),
                    ...
                    return this.SqlSyntax.GetStringColumnWildcardComparison(col, this.SqlParameters.Count - 1, columnType);
                case "Contains":
                    this.SqlParameters.Add(string.Format("{0}{1}{0}",
                        this.SqlSyntax.GetWildcardPlaceholder(),
                        this.RemoveQuote(val)));
                    return this.SqlSyntax.GetStringColumnWildcardComparison(col, this.SqlParameters.Count - 1, columnType);
                case "InvariantEquals":
                case "SqlEquals":
                    //recurse
                    return this.HandleStringComparison(col, val, "Equals", columnType);
                case "InvariantStartsWith":
                case "SqlStartsWith":
                    //recurse
                    return this.HandleStringComparison(col, val, "StartsWith", columnType);
                case "InvariantEndsWith":
                case "SqlEndsWith":
                    //recurse
                    return this.HandleStringComparison(col, val, "EndsWith", columnType);
                case "InvariantContains":
                case "SqlContains":
                    //recurse
                    return this.HandleStringComparison(col, val, "Contains", columnType);
                default:

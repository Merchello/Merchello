namespace Merchello.Core.Cache
{
    ...
    internal abstract class DictionaryCacheProviderBase : ICacheProvider
    {
        ...
        public virtual void ClearAllCache()
        {
            ...
            {
                foreach (var entry in this.GetDictionaryEntries()
                    .ToArray())
                    this.RemoveEntry((string) entry.Key);
            }
        }
        ...
        public virtual IEnumerable<object> GetCacheItemsByKeySearch(string keyStartsWith)
        {
            ...
            using (this.ReadLock)
            {
                entries = this.GetDictionaryEntries()
                    .Where(x => ((string)x.Key).Substring(plen).InvariantStartsWith(keyStartsWith))
        ...
        public virtual object GetCacheItem(string cacheKey)
        {
            ...
            using (this.ReadLock)
            {
                result = this.GetEntry(cacheKey) as Lazy<object>; // null if key not found
            }

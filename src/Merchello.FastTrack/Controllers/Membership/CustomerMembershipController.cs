namespace Merchello.FastTrack.Controllers.Membership
{
    ...
    public class CustomerMembershipController : CustomerMembershipControllerBase
    {
        ...
        /// </returns>
        [ChildActionOnly]
        public virtual ActionResult LoginForm(string view = "", string successRedirectUrl = "")
        {
            var model = new LoginModel { RememberMe = true, SuccessRedirectUrl = successRedirectUrl };
        ...
        public virtual ActionResult ForgotPassword(ForgotPasswordModel model)
        {
            ...
            var newPassword = user.ResetPassword(user.PasswordQuestion);

            // assumes you have set the SMTP settings in web.config and supplied a default "from" email
            var msg = new MailMessage
            {
                From = new MailAddress(ConfigurationManager.AppSettings["ResetPassword:EmailFrom"]),
